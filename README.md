...
Contact [donavinliebgott@gmail.com](mailto:donavinliebgott@gmail.com) to
...
| File | Place here |
|---|---|
| `license_check.so` | Same directory as `scmap.py` |
| `scmap.crt` | `~/.seiscomp/licenses/` or `~/seiscomp/share/licenses/` |
...
If the `.so` or `.crt` is missing, scmap falls back to the community
...
- **Pro edition** — paid, watermark‑free, delivered as a compiled
  license module.

SeisComP is distributed under the
