...
# ── License module ─────────────────────────────────────────────────────────
# Pro edition uses the compiled license_check.so shipped to paying customers.
# Community edition uses _license.py stub (always unlicensed).
# The import order ensures _license_pro is tried first.
try:
    from _license_pro import is_licensed, customer_name, error as _license_error
    _LICENSED = is_licensed()
except Exception:
    from _license import is_licensed, customer_name
    _LICENSED = False
    _license_error = lambda: None
...
class MapBuilder:
    ...
    def _check_watermark(self, fig):
        """Draw watermark if unlicensed — check is in compiled .so."""
        if _LICENSED:
            return
    ...
    def _draw_wadati_diagram(self, output_path, config):
        ...
        # Compute default extent (from user params or first-event fallback)
        extent = self._compute_extent()
        if not extent:
            seiscomp.logging.warning("Wadati: no extent computed")
            return
        lon_min, lon_max, lat_min, lat_max = extent

        # Collect events and stations within the default extent
        local_events = [se for se in self.events
                        if se.latitude is not None
                        and lon_min <= se.longitude <= lon_max
                        and lat_min <= se.latitude <= lat_max]
        local_station_lats = []
        local_station_lons = []
        local_station_keys = set()
        for se in local_events:
            for ph, dist, az, *_ in se.arrivals:
                if dist is not None and az is not None:
                    ...
                    if key not in local_station_keys:
                        local_station_keys.add(key)
                        local_station_lats.append(slat)
                        local_station_lons.append(slon)

        # Auto-zoom extent from local data unless --region was given
        has_explicit = bool(config.get('region'))
        if not has_explicit and (local_events or local_station_lats):
            all_lats = ([se.latitude for se in local_events]
                        + local_station_lats)
            all_lons = ([((se.longitude + 180) % 360) - 180
                         for se in local_events]
                        + [((lon + 180) % 360) - 180
                           for lon in local_station_lons])
            z_lat_min, z_lat_max = min(all_lats), max(all_lats)
            z_lon_min, z_lon_max = min(all_lons), max(all_lons)
            lat_range = max(z_lat_max - z_lat_min, 0.5)
            lon_range = max(z_lon_max - z_lon_min, 0.5)
            pad_lat = lat_range * 0.2
            pad_lon = lon_range * 0.2
            lon_min = z_lon_min - pad_lon
            lon_max = z_lon_max + pad_lon
            lat_min = z_lat_min - pad_lat
            lat_max = z_lat_max + pad_lat
            seiscomp.logging.debug(
                "Wadati: auto extent from events+stations "
                "lon=[%.4f,%.4f] lat=[%.4f,%.4f]"
                % (lon_min, lon_max, lat_min, lat_max))

        # Filter all events to the final extent
        ...
        n_vel = len(vp_vals)
        n_events_wadati = len([se for se in self.events
                               if se.arrivals and se.latitude is not None
                               and se.origin_time is not None])
        ...
        ax.scatter(tp, tsp, s=35, c='#2196F3', edgecolors='#1565C0',
                   ...
                   label=r'Observed P/S pairs ($n$=%d)' % n_pts)
        ...
        if n_ponly > 0:
            ax.scatter(tp_ponly, tsp_ponly, s=40, marker='s',
                       ...
                       label=r'P-only $\times$ model S–P ($n$=%d)' % n_ponly)
        ...
        subtitle = (
            ...
            f'Paired P/S: {n_pts}  |  '
            + (f'P-only + model S: {n_ponly}  |  ' if n_ponly else '')
            + ('Green = theoretical model' if has_theo else '')
        )
...
""")
    ...
    def run(self):
        ...
        if _LICENSED:
            seiscomp.logging.info(
                "Licensed to %s" % customer_name())
        else:
            err = _license_error() if '_license_error' in dir() else None
            seiscomp.logging.warning(
                "UNLICENSED — watermark will be applied (%s)"
                % (err or 'no license found'))
